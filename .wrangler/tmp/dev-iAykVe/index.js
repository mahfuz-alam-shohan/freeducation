...
var renderSubjectRows = /* @__PURE__ */ __name((subjects) => {
  ...
  return subjects.map((subject) => {
    return `
        <tr>
          <td>
            <a class="table-link" href="${escapeValue9(subject.manageUrl)}">${escapeValue9(subject.name)}</a>
            <span class="table-meta">${escapeValue9(subject.manageLabel)}</span>
          </td>
          <td class="table-actions">
            <a class="button-link" href="${escapeValue9(subject.manageUrl)}">${escapeValue9(subject.manageLabel)}</a>
          </td>
        </tr>`;
  }).join("");
}, "renderSubjectRows");
  ...
  const rows = users.length ? users.map(
    (user, index) => `
        <tr>
          ...
          <td>${escapeValue10(user.name)}</td>
          <td>${escapeValue10(user.email)}</td>
          <td>${escapeValue10(new Date(user.createdAt).toLocaleDateString())}</td>
          <td>
            <div class="confirm-delete">
...
var RATE_LIMITS = {
  AUTH: { windowMs: 15 * 60 * 1e3, maxRequests: 5 },
  // 5 requests per 15 minutes
  GENERAL: { windowMs: 15 * 60 * 1e3, maxRequests: 100 },
  // 100 requests per 15 minutes
  API: { windowMs: 1 * 60 * 1e3, maxRequests: 30 }
...
// .wrangler/tmp/bundle-pqj8JA/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-pqj8JA/middleware-loader.entry.ts
