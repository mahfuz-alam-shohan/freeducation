  ...
  return `
    <div class="card table-card">
      <div class="table-header">
        <div>
          <h3>${topic ? topic.name : chapter.name} - ${title}</h3>
          <p>${subject.name} > ${nodeName}${topic ? ` > ${chapter.name}` : ''}</p>
        </div>
        <div class="table-actions">
          <a class="button ghost" href="${backHref}">Cancel</a>
        </div>
      </div>
      <form class="table-body question-form" data-form="question-page" data-owner="${topic ? 'topic' : 'chapter'}" data-owner-id="${topic ? topic.id : chapter.id}" data-mode="${mode}" data-type="${typeKey}" data-section="${sectionKey || ''}" data-id="${formId}" data-back-href="${backHref}">
        <div class="form-grid">
          <div class="field">
...
export function renderMcqForm(config) {
  ...
  return `
    <div class="card table-card">
      ...
      <form class="table-body question-form mcq-form" data-form="question-page" data-owner="${topic ? 'topic' : 'chapter'}" data-owner-id="${topic ? topic.id : chapter.id}" data-mode="${mode}" data-type="MCQ" data-id="${formId}" data-back-href="${backHref}">
        <div class="form-grid">
          <div class="field">
            <label>Question</label>
