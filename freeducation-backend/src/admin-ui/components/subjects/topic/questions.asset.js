  ...
  const rows = [
    {
      ...
      href: `#subjects/${subject.id}/chapters/${node.id}/${chapter.id}/topics/${topic.id}/questions/cq`
    },
    {
      key: 'type-MCQ',
      title: safeLabels.types.MCQ,
      description: 'MCQ questions for this topic.',
      count: mcqQuestions.length,
      href: `#subjects/${subject.id}/chapters/${node.id}/${chapter.id}/topics/${topic.id}/questions/mcq`
    }
  ].map((item) => `
      <tr>
  ...
  return `
    <div class="card table-card">
      <div class="table-header">
        <div>
          <h3>${topic.name} - Question Bank</h3>
...
export function renderTopicQuestionCQOverview(detail) {
  const { topic, chapter, subject, node, labels, questions } = detail;
  const nodeName = node.displayName || node.serverName || '';
  const safeLabels = {
    types: { ...DEFAULT_TYPE_LABELS, ...(labels ? labels.types : {}) },
  ...
  const rows = sectionKeys.map((key) => {
    ...
    return `
      <tr>
        ...
        <td class="cell-actions">
          <a class="button secondary" href="#subjects/${subject.id}/chapters/${node.id}/${chapter.id}/topics/${topic.id}/questions/cq/${key}">Open</a>
          <button class="button ghost" data-action="labels-save">Save title</button>
        </td>
      </tr>
    `;
  ...
  return `
    <div class="card table-card">
      <div class="table-header">
        <div>
          <h3>${topic.name} - ${safeLabels.types.CQ}</h3>
          <p>${subject.name} > ${nodeName} > ${chapter.name}</p>
        </div>
        <div class="table-actions">
          <button class="button secondary" data-action="topic-edit">Edit topic</button>
          <a class="button ghost" href="#subjects/${subject.id}/chapters/${node.id}/${chapter.id}/topics/${topic.id}/questions">Back</a>
        </div>
      </div>
      <div class="table-scroll">
        <table class="table">
...
export function renderTopicQuestionCQSection(detail, sectionKey, formState = null) {
  const { topic, chapter, subject, node, labels, questions } = detail;
  const nodeName = node.displayName || node.serverName || '';
  ...
  const baseHref = `#subjects/${subject.id}/chapters/${node.id}/${chapter.id}/topics/${topic.id}/questions/cq/${normalizedKey}`;
  ...
  return `
    ...
    <div class="card table-card">
      <div class="table-header">
        <div>
          <h3>${topic.name} - ${sectionTitle}</h3>
          <p>${subject.name} > ${nodeName} > ${chapter.name}</p>
        </div>
        <div class="table-actions">
          <a class="button" href="${addHref}">Add ${sectionTitle}</a>
          <button class="button secondary" data-action="topic-edit">Edit topic</button>
          <a class="button ghost" href="#subjects/${subject.id}/chapters/${node.id}/${chapter.id}/topics/${topic.id}/questions/cq">Back</a>
        </div>
      </div>
      <div class="table-scroll">
        <table class="table">
...
export function renderTopicQuestionMCQ(detail, formState = null, mediaUrl) {
  ...
  return `
    ...
    <div class="card table-card">
      <div class="table-header">
        <div>
          <h3>${topic.name} - ${safeLabels.types.MCQ}</h3>
          <p>${subject.name} > ${nodeName} > ${chapter.name}</p>
        </div>
        <div class="table-actions">
