...
export function renderNavigationGroup(group, active) {
  if (group.collapsible) {
    ...
    const items = group.items.map((item) => `<a href="${item.href}" class="submenu-item ${active === item.key ? "active" : ""}"><span class="icon">${item.icon}</span><span class="label">${item.label}</span></a>`).join("");
  ...
  const items = group.items.map((item) => `<a href="${item.href}" class="menu-item ${active === item.key ? "active" : ""}"><span class="icon">${item.icon}</span><span class="label">${item.label}</span></a>`).join("");
