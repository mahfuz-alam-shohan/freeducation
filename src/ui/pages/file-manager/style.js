...
.fm-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(178px,1fr));gap:6px}
.fm-item{border:1px solid var(--border);background:var(--surface);border-radius:10px;overflow:hidden;display:grid;grid-template-rows:auto 1fr;min-height:234px;opacity:0;transform:translateY(10px) scale(.98);animation:fm-card-in .44s var(--motion-spring) forwards;animation-delay:calc(var(--item-index,0) * 22ms)}
.fm-meta{padding:7px;border-bottom:1px solid var(--border);display:grid;gap:2px;background:var(--surface-soft)}
.fm-name{font-size:.74rem;line-height:1.25;word-break:break-word}
.fm-detail{font-size:.71rem;color:var(--text-muted);display:flex;justify-content:space-between;gap:4px}
.fm-thumb{position:relative;min-height:160px;background:var(--surface-strong);display:grid;place-items:center;overflow:hidden}
...
.fm-pdf-icon{width:62px;height:62px;opacity:.86}
.fm-type-badge{position:absolute;right:6px;bottom:6px;font-size:.68rem;padding:2px 6px;border-radius:999px;background:color-mix(in srgb,var(--surface) 75%,transparent);border:1px solid var(--border)}
...
@media (max-width:820px){.fm-filters{grid-template-columns:1fr}.fm-grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:4px}.fm-item{border-radius:8px;min-height:210px}}
@media (max-width:620px){.fm-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.fm-meta{padding:6px}}
