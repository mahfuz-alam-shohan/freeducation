...
.dash-card{background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:9px;display:grid;gap:4px;min-height:90px;animation:panel-in .2s ease both}
...
.dash-grid.is-loading .dash-kpi::after{content:'';display:block;height:30px;width:60%;border-radius:6px;background:linear-gradient(90deg,var(--surface-soft),color-mix(in srgb,var(--accent) 26%,var(--surface-soft)),var(--surface-soft));background-size:200% 100%;animation:dash-skeleton 1s linear infinite}
