...
(() => {
  ...
  const mediaSourcesForPost = (post, options = {}) => {
    ...
    const uniqueUrls = [];
    const seen = new Set();
    normalizedUrls.forEach((src) => {
      if (seen.has(src)) return;
      seen.add(src);
      uniqueUrls.push(src);
    });
    return uniqueUrls.filter((src) => !isBrokenImage(src)).slice(0, Math.max(1, maxImages));
  };

  const syncPostMediaLayout = (mediaRoot) => {
    if (!(mediaRoot instanceof HTMLElement)) return;
    const items = Array.from(mediaRoot.querySelectorAll(':scope > .post-media-item'));
    ...
    items.forEach((item) => {
      ...
      const baseSrc = String(image.dataset.baseSrc || '').trim();
    ...
    const visibleItems = items.filter((item) => String(item.style.display || '').toLowerCase() !== 'none');
    const visibleCount = visibleItems.length;

    mediaRoot.classList.remove('post-media-grid-1', 'post-media-grid-2', 'post-media-grid-3', 'post-media-grid-4', 'post-media-single');
    if (visibleCount <= 0) return;

    const normalizedCount = Math.min(4, Math.max(1, visibleCount));
    mediaRoot.classList.add('post-media-grid-' + normalizedCount);
    if (normalizedCount === 1) {
      mediaRoot.classList.add('post-media-single');
      items.forEach((item) => {
        if (item !== visibleItems[0]) item.style.display = 'none';
      });
    }
  };

  window.syncPostMediaLayout = syncPostMediaLayout;
  ...
  const mediaMarkupForPost = (post, options = {}) => {
    ...
    const maxImages = Number.isInteger(Number(options.maxImages)) ? Number(options.maxImages) : 3;
    const allRenderableUrls = mediaSourcesForPost(post, { isBrokenImage, maxImages: Number.MAX_SAFE_INTEGER });
    if (!allRenderableUrls.length) return '';
    const renderableUrls = allRenderableUrls.slice(0, Math.max(1, maxImages));
    const hiddenCount = Math.max(0, allRenderableUrls.length - renderableUrls.length);
    ...
    const items = renderableUrls.map((baseImageSrc, index) => {
      const imageSrc = baseImageSrc
        ...
        : '';
      const imageTag = '<img class="post-image" src="' + escapeHtml(imageSrc) + '" data-base-src="' + escapeHtml(baseImageSrc) + '" alt="Post image" loading="lazy" decoding="async" fetchpriority="low" onload="if(window.syncPostMediaLayout)window.syncPostMediaLayout(this.closest(\\'.post-media\\'));" onerror="' + escapeHtml(brokenMarker) + '(this.dataset.baseSrc);this.closest(\\'.post-media-item\\').style.display=\\'none\\';if(window.syncPostMediaLayout)window.syncPostMediaLayout(this.closest(\\'.post-media\\'));">';
      const href = mediaHrefForIndex ? String(mediaHrefForIndex(post, index) || '') : '';
      const content = href
        ? '<a class="post-media-link" href="' + escapeHtml(href) + '">' + imageTag + '</a>'
        : imageTag;
      const hasOverflow = hiddenCount > 0 && index === (renderableUrls.length - 1);
      const overflowBadge = hasOverflow
        ? '<span class="post-media-overflow">+' + hiddenCount + '</span>'
        : '';
      return '<figure class="post-media-item' + (hasOverflow ? ' has-overflow' : '') + '">' + content + overflowBadge + '</figure>';
    }).join('');
