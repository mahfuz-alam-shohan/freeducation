...
export const handleStudentProfile = async (request: Request, env: Env, path: string): Promise<Response | null> => {
  ...
  if (path === '/api/student/profile' && request.method === 'PUT') {
    ...
    const existing = await env.DB
      .prepare('SELECT class_label, group_label, religion, date_of_birth, batch_year, points FROM users WHERE id = ?')
      .bind(payload.id)
      .first();
    ...
    await env.DB
      .prepare('UPDATE users SET class_label = ?, group_label = ?, religion = ?, date_of_birth = ?, batch_year = ? WHERE id = ?')
