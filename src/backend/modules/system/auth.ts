...
export const handleAuth = async (request: Request, env: Env, path: string): Promise<Response | null> => {
  ...
  if (path === '/api/register-admin' && request.method === 'POST') {
    ...
    await env.DB.batch([
      env.DB
        .prepare('INSERT INTO users (username, name, email, password_hash, role) VALUES (?, ?, ?, ?, ?)')
        .bind(cleanedUsername, cleanedUsername, null, passwordHash, 'admin'),
      env.DB
        .prepare('INSERT INTO admin_permissions (user_id, permissions) VALUES ((SELECT id FROM users WHERE username = ?), ?)')
  ...
  if (path === '/api/login' && request.method === 'POST') {
    ...
    let user = await env.DB
      ...
      .bind(cleanedUsername, normalizedEmail)
      .first();
  ...
  if (path === '/api/me' && request.method === 'GET') {
    ...
    const userRow = payload
      ? await env.DB.prepare('SELECT username, role, class_label, group_label FROM users WHERE id = ?').bind(payload.id).first()
      : null;
    return Response.json(
      {
        user: payload
