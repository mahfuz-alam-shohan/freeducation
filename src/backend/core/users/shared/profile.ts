...
import { apiHeaders, getAuthPayload, recordEditHistory, requireJwtSecret } from './utils';
...
export const handleProfile = async (request: Request, env: Env, path: string): Promise<Response | null> => {
  ...
  const getTokenPayload = async () => {
    const tokenPayload = await getAuthPayload(request, env);
  ...
  if (path === '/api/profile' && request.method === 'GET') {
    ...
    const assignmentRow =
      payload.role === 'teacher'
        ? await env.DB.prepare('SELECT level, subject FROM teacher_assignments WHERE user_id = ?').bind(payload.id).first()
        : null;
    const avatarRow = await env.DB.prepare('SELECT avatar_key, avatar_content_type, updated_at FROM user_profiles WHERE user_id = ?')
      .bind(payload.id)
      .first();
  ...
  if (path === '/api/profile/avatar' && request.method === 'POST') {
    ...
    const existing = await env.DB.prepare('SELECT avatar_key FROM user_profiles WHERE user_id = ?').bind(payload.id).first();

    await env.DB
      .prepare(
        'INSERT INTO user_profiles (user_id, avatar_key, avatar_content_type) VALUES (?, ?, ?) ' +
  ...
  if (path === '/api/profile/history' && request.method === 'GET') {
    ...
    const rows = await env.DB.prepare('SELECT action, details, created_at FROM edit_history WHERE user_id = ? ORDER BY created_at DESC LIMIT 20')
      .bind(payload.id)
      .all();
