...
teacherRoutes.put('/profile', async (c) => {
  ...
  await c.env.DB.batch([
    c.env.DB.prepare('UPDATE users SET email = ? WHERE id = ?').bind(email || existing.email, payload.id),
    c.env.DB
      .prepare(
      ...
      .bind(payload.id, (email || existing.email) as string, name || existing.name),
  ]);
