...
export const handleAuth = async (request: Request, env: Env, path: string): Promise<Response | null> => {
  ...
  if (path === '/api/register-admin' && request.method === 'POST') {
    ...
    await env.DB.prepare('INSERT INTO users (email, password_hash, role) VALUES (?, ?, ?)').bind(null, passwordHash, 'admin').run();
    const adminRow = await env.DB.prepare("SELECT id FROM users WHERE role = 'admin' ORDER BY id DESC LIMIT 1").first();
    if (!adminRow?.id) {
      return Response.json({ success: false, error: 'Admin creation failed.' }, { status: 500, headers: apiHeaders });
    }

    await env.DB.batch([
      env.DB.prepare('INSERT INTO user_profiles (user_id, username, name) VALUES (?, ?, ?)').bind(adminRow.id, cleanedUsername, cleanedUsername),
      env.DB
        .prepare('INSERT INTO admin_permissions (user_id, permissions) VALUES (?, ?)')
        .bind(adminRow.id, JSON.stringify(['dashboard', 'classes', 'settings', 'thumbnails', 'userManagement'])),
  ...
  if (path === '/api/login' && request.method === 'POST') {
    ...
    let user = await env.DB
      ...
      .bind(normalizedEmail, cleanedUsername)
      .first();
    ...
    if (user) {
      ...
      if (role === 'teacher') {
        ...
        const permissionsRow = await env.DB
      ...
      if (role === 'student') {
        const academicRow = await env.DB
          .prepare('SELECT class_label, group_label FROM academic_profiles WHERE user_id = ?')
          .bind(user.id)
          .first();
        classLabel = (academicRow?.class_label as string) || null;
        groupLabel = (academicRow?.group_label as string) || null;
      }
    } else {
  ...
  if (path === '/api/me' && request.method === 'GET') {
    ...
    const userRow = payload
      ? await env.DB
          ...
          .first()
      : null;
    const academicRow = payload
      ? await env.DB.prepare('SELECT class_label, group_label FROM academic_profiles WHERE user_id = ?').bind(payload.id).first()
      : null;
    return Response.json(
      {
        user: payload
          ? {
              ...
              classLabel: academicRow?.class_label || payload.classLabel || null,
              groupLabel: academicRow?.group_label || payload.groupLabel || null,
            }
          : null,
      },
      { headers: apiHeaders }
