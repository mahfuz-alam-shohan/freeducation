...
export async function handleAdminRequest(request: Request, env: Env): Promise<Response> {
  ...
  // Question List View for a Chapter & Type
...
// ... (renderClassesList, renderClassDetail are mostly same, just updating linking logic if needed. Assuming existing code is fine for those)
// Re-implementing compact Subject Detail to link to Chapter Detail
  ...
  return renderPage("Classes", `
    ...
    <div class="inset-list">
      ${classes.results?.map(c => `
        <div class="list-row" onclick="window.location='/admin/classes/${c.id}'">
          <div class="row-content">
            <div class="row-title">${escapeHtml(c.name)}</div>
            <div class="row-subtitle">${c.subject_count} Subjects ${c.has_groups?'• Groups':''}</div>
          </div>
          <div class="row-action">›</div>
        </div>
      `).join('') || '<div style="padding:16px;text-align:center;color:var(--text-secondary);">No classes</div>'}
    </div>
    <!-- Modals omitted for brevity, assume existing -->
    <div id="new-class-modal" class="modal-overlay">
      <div class="modal-card">
        <div class="modal-header">New Class</div>
        <form action="/admin/classes" method="POST">
           <div class="modal-body"><div class="input-group"><input name="name" class="input" required placeholder="Class Name"></div><div class="input-group" style="display:flex;justify-content:space-between;"><span>Groups</span><input type="checkbox" name="has_groups" value="1" style="width:20px;height:20px;"></div></div>
           <div class="modal-actions"><div class="modal-btn" onclick="toggleModal('new-class-modal',false)">Cancel</div><button class="modal-btn">Create</button></div>
        </form>
      </div>
    </div>
  `, "classes", session);
...
async function renderClassDetail(session: any, env: Env, classId: number) {
  // ... (Full Class Detail logic from previous step)
  // Just a placeholder to call the previous logic or assume it exists. 
  // I will output the *Subject Detail* which is the entry point to Chapters.
  
  // RERUN of specific code needed for completeness:
  const classData = await env.DB.prepare("SELECT * FROM classes WHERE id = ?").bind(classId).first<ClassRow>();
  if(!classData) return new Response("Not found", {status:404});

  const subjects = await env.DB.prepare("SELECT * FROM subjects WHERE class_id = ? OR link_id = (SELECT link_id FROM class_link_members WHERE class_id = ?)").bind(classId, classId).all<SubjectRow>();
  
  return renderPage(classData.name, `
    <div class="header">
      <h1 class="page-title">${escapeHtml(classData.name)}</h1>
      <div class="page-subtitle">Subjects</div>
    </div>
    <div style="padding:0 1rem; margin-bottom:1rem; text-align:right;">
       <button onclick="openEdit('new-subject-modal', '/admin/subjects', {class_id: ${classId}, group_id: ''})" class="btn-text">+ Subject</button>
    </div>
    <div class="inset-list">
       ${subjects.results?.map(s => `
         <div class="list-row" onclick="window.location='/admin/subjects/${s.id}'">
            <div class="row-content"><div class="row-title">${escapeHtml(s.name)}</div></div>
            <div class="row-action">›</div>
         </div>
       `).join('')}
    </div>
...
async function renderSubjectDetail(session: any, env: Env, subjectId: number) {
  ...
  const chapters = await env.DB.prepare(`
    ...
    FROM chapters c WHERE subject_id = ? ORDER BY sort_order ASC, created_at ASC
  ...
  return renderPage(subject.name, `
    ...
    <div class="inset-list">
      ${chapters.results?.map((ch, idx) => `
        <div class="list-row" onclick="window.location='/admin/chapters/${ch.id}'">
           ...
           <div class="row-content">
             ...
             <div class="row-subtitle">${ch.topic_count} Topics • ${ch.question_count} Questions</div>
           </div>
           <div class="row-action">›</div>
        </div>
...
// --- NEW: Topic Detail (Content Manager) ---
...
// ... (renderSettings, renderLogin same as before)
...
// --- Logic Handlers (Standard CRUD) ---
// (Previous handlers + New ones)
