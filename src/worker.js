...
async function handleAdminPost(request, env, url, user = null) {
  ...
  if (url.pathname === "/api/exams/start" && request.method === "POST") {
    const form = await request.formData();
    const subjectId = String(form.get("subjectId") || "");
    const requestedNodeId = String(form.get("subjectNodeId") || "");
    const nodeRows = await env.DB.prepare("SELECT id, supports_chapters, node_type, sort_order FROM subject_nodes WHERE subject_id = ?1 ORDER BY sort_order ASC, server_name ASC").bind(subjectId).all();
    const nodes = nodeRows.results || [];
    const chapterNodes = nodes.filter((node) => Number(node.supports_chapters) === 1 && node.node_type !== "content");
    const defaultNode = chapterNodes[0] || nodes.find((node) => node.node_type !== "content") || null;
    const selectedNode = nodes.find((node) => node.id === requestedNodeId) || defaultNode;
    if (!selectedNode) return redirect(`/learn/subjects/${subjectId}/exam`);
    const subjectNodeId = selectedNode.id;
    ...
    const scopeForQuery = {
      subjectId,
      subjectNodeId: scope === "subject" ? null : subjectNodeId,
...
async function handleDynamicPages(url, env, user) {
  ...
  if (examSetupMatch) {
    ...
    const allNodeRows = await env.DB
      .prepare("SELECT id, display_name, supports_chapters, node_type, sort_order FROM subject_nodes WHERE subject_id = ?1 ORDER BY sort_order ASC, server_name ASC")
      .bind(subject.id)
      .all();
    const allNodes = allNodeRows.results || [];
    const chapterNodes = allNodes.filter((node) => Number(node.supports_chapters) === 1 && node.node_type !== "content");
    const books = chapterNodes.length > 0 ? chapterNodes : allNodes.filter((node) => node.node_type !== "content");
    ...
    const [subjectCount, chapterCount, topicCount] = await Promise.all([
      countScopedMcqs(env.DB, { subjectId: subject.id, subjectNodeId: null, chapterId: null, topicId: null }),
