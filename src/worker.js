...
async function handleDynamicPages(url, env, user) {
  ...
  if (publicSubjectRootMatch) {
    ...
    if (nodes.length === 1 && nodes[0].supports_chapters) {
      ...
      return html(withFloatingExamButton(publicSubjectNodePage(user, subject, subject.name, "Choose a chapter.", chapters, (chapter) => `/learn/subjects/${subject.id}/nodes/${rootNode.id}/chapters/${chapter.id}`), `/learn/subjects/${subject.id}/exam?node=${rootNode.id}`));
    }
    return html(withFloatingExamButton(publicSubjectNodePage(user, subject, subject.name, "Select a book.", nodes, (node) => `/learn/subjects/${subject.id}/nodes/${node.id}`), `/learn/subjects/${subject.id}/exam`));
  }
  ...
  if (publicSubjectNodeMatch) {
    ...
    if (node.supports_chapters) {
      ...
      return html(withFloatingExamButton(publicSubjectNodePage(user, subject, node.display_name, "Choose a chapter.", chapters, (chapter) => `/learn/subjects/${subject.id}/nodes/${node.id}/chapters/${chapter.id}`), `/learn/subjects/${subject.id}/exam?node=${node.id}`));
    }
    ...
    if (sectionChildren.length > 0) {
      return html(withFloatingExamButton(publicSubjectNodePage(user, subject, node.display_name, "Choose a section.", sectionChildren, (child) => `/learn/subjects/${subject.id}/nodes/${child.id}`), `/learn/subjects/${subject.id}/exam`));
    }
  ...
  if (publicChapterMatch) {
    ...
    if (chapter.has_topics) {
      ...
      return html(withFloatingExamButton(publicSubjectNodePage(user, subject, chapter.name, "Choose a topic.", topics, (topic) => `/learn/subjects/${subject.id}/nodes/${node.id}/chapters/${chapter.id}/topics/${topic.id}`), `/learn/subjects/${subject.id}/exam?node=${node.id}&chapter=${chapter.id}`));
    }
