...
export async function createSession(db, session) {
  await db.prepare("INSERT INTO sessions (id, user_id, expires_at, created_at) VALUES (?1, ?2, ?3, ?4)").bind(session.id, session.userId, session.expiresAt, session.createdAt).run();
}
...
export async function listUsers(db) {
  const rows = await db.prepare("SELECT id, name, email, role, created_at FROM users ORDER BY created_at DESC LIMIT 500").all();
  return rows.results ?? [];
}
...
export async function updateUserPassword(db, userId, passwordHash, passwordSalt, passwordIterations) {
  await db.prepare("UPDATE users SET password_hash = ?2, password_salt = ?3, password_iterations = ?4, updated_at = ?5 WHERE id = ?1").bind(userId, passwordHash, passwordSalt, passwordIterations, nowIso()).run();
}
