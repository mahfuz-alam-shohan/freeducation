...
export async function ensureSchema(db, options = {}) {
  ...
  for (const tableName of Object.keys(TABLES)) {
    await db.exec(TABLES[tableName].create);
  }
  ...
  const shouldCleanUnknownTables = options.cleanUnknownTables === true;
  if (shouldCleanUnknownTables) {
    for (const existingTable of existingNames) {
      if (!(existingTable in TABLES)) {
        await db.exec(`DROP TABLE IF EXISTS ${existingTable}`);
      }
    }
  }
