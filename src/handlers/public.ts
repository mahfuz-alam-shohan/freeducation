...
export const renderHome = async (env: Bindings) => {
  const classes = await dbAll(env, 'SELECT * FROM classes');
  const body = `
    <div style="background:linear-gradient(135deg, var(--primary), var(--secondary)); color:white; padding:4rem 0; text-align:center;">
      <div class="container">
        <h1 style="font-size:2.5rem; margin-bottom:1rem;">Master Your Curriculum</h1>
    ...
    <div class="container" style="margin-top:-2rem;">
      <div class="grid-3">
        ${classes.map((c:any) => `
          <a href="/class/${c.id}" class="card" style="text-align:center; padding:2rem;">
            <div style="font-size:2rem; margin-bottom:0.5rem;">🎓</div>
            <h3>${c.name}</h3>
            <p class="text-muted">${c.level}</p>
          </a>
        `).join('')}
      </div>
    </div>
...
export const renderClass = async (env: Bindings, id: number) => {
  ...
  const body = `
    <div class="container" style="padding:2rem 1rem;">
      <h2>${cls.name} <span class="text-muted" style="font-size:1rem;">Subjects</span></h2>
      <div class="grid-3" style="margin-top:1.5rem;">
        ${subjects.map((s:any) => `
          <a href="/subject/${s.id}" class="card">
            <h3>${s.name}</h3>
            <p class="text-muted" style="margin-top:0.5rem;">${s.description || 'View Chapter & Resources'}</p>
    ...
    <div class="container" style="padding:2rem 1rem;">
      ...
      <div class="grid-2">
        <div>
          ...
          <div class="stack">
            ${chapters.map((c:any) => `
              <a href="/chapter/${c.id}" class="card" style="display:flex; justify-content:space-between; align-items:center;">
                <div>
        ...
        <div>
          ...
          <div class="card stack">
            ...
            ${resources.map((r:any) => `
              <a href="/resource/${r.id}" target="_blank" style="display:flex; justify-content:space-between; padding:0.5rem; border-bottom:1px solid #f1f5f9;">
                <span>${r.title}</span>
                <span class="badge badge-green">${r.category}</span>
              </a>
            `).join('')}
          </div>
        </div>
  ...
  const body = `
    <div class="container" style="padding:2rem 1rem;">
      <div style="text-align:center; margin-bottom:3rem;">
        ...
        <h1 style="font-size:2.5rem; color:var(--primary);">${chapter.name}</h1>
      </div>
        ...
        ${topics.map((t:any) => {
           ...
           return `
             <div id="topic-${t.id}" style="margin-bottom:3rem;">
               <h2 style="border-bottom:2px solid var(--slate-200); padding-bottom:0.5rem; margin-bottom:1.5rem;">${t.title}</h2>
          ...
          <div class="grid-2">
             ${questions.map((q:any) => `
               <div class="card">
                 ...
                 <div style="font-weight:600; margin-top:0.5rem;">${q.question_text}</div>
                 ${q.type === 'mcq' ? `<div class="text-muted" style="margin-top:0.5rem; font-size:0.9rem;">${JSON.parse(q.options_json).map((o:string, i:number)=>`<div>${String.fromCharCode(65+i)}) ${o}</div>`).join('')}</div>` : ''}
